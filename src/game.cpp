...
bool Game::flood_peer(GamePeer *game_peer) {
    ...
    if (auto peer = game_peer->peer.lock()) {
        ...
        for (const auto& event : event_cache) {
            ...
            // Serialize event
            photon::EventData event_data{.code = event.code, .parameters = event.top_params};
            ...
            const auto event_payload = photon::serialize_event_data(event_data);
            peer->send(event_payload, enet::EnetSendOptions{0});
...
void Game::broadcast_event(Event& event) {
    ...
    if (event.serialization_cache.empty()) {
        photon::EventData event_data{.code = event.code, .parameters = std::move(event.top_params)};
        if (event.sender_actor_id)
            event_data.parameters[DictKeyCodes::GameAndActor::ActorNo] = static_cast<int32_t>(event.sender_actor_id);
        if (!event.data.is_null())
            event_data.parameters[DictKeyCodes::RoutingAndEvents::Data] = std::move(event.data);
        event.serialization_cache = photon::serialize_event_data(event_data);
    }
    const auto& event_payload = event.serialization_cache;

    enet::EnetSendOptions send_options{.channel = 0, .mode = event.delivery_mode};
        ...
        if (*receiver_group == ReceiverGroup::MasterClient) {
            ...
            if (auto *game_peer = find_peer(master_actor))
                if (game_peer->has_interest_group(event.interest_group))
                    if (auto peer = game_peer->peer.lock())
                        peer->send(event_payload, send_options);
        } else {
...
photon::Value Game::get_game_prop(const photon::Value& key) {
...
photon::Hashtable Game::get_basic_game_props() {
    photon::Hashtable fres;
...
photon::Hashtable Game::get_game_props(bool no_custom) {
    auto fres = no_custom ? photon::Hashtable{} : custom_props;
...
photon::Hashtable Game::get_actor_props() {
    photon::Hashtable fres;
...
void Game::insert_game_props(photon::Hashtable update) {
...
bool Game::expect_game_props(photon::Hashtable expected) {
    bool ok = true;
#define PROP_MAP_ENTRY(game_param, type, var, updates_lobby)                                                                                                   \
...
void Game::insert_actor_props(int32_t actor_id, const photon::Hashtable& update) {
...
bool Game::expect_actor_props(int32_t actor_id, const photon::Hashtable& expected) {
...
bool Game::matches_filter(const photon::Value& event_data, const photon::Hashtable& filter) {
    // If filter is empty, it's a match
    if (filter.empty())
        return true;

    // If event has no data but filter is not empty, no match
    if (!event_data.is<photon::HashtablePtr>())
        return false;

    const auto& data_ptr = event_data.get<photon::HashtablePtr>();
    if (!data_ptr)
        return false;
